...
def get_gpu_info():
    ...
    # Check for /dev/dri (Linux Direct Rendering Infrastructure)
    if os.path.exists('/dev/dri'):
        print("- **Direct Rendering Infrastructure (/dev/dri) found**")
        try:
            cards = os.listdir('/dev/dri')
            print(f"  - Devices: {', '.join(cards)}")
            gpu_found = True
        except Exception as e:
            print(f"  - Error listing devices: {e}")

    if not gpu_found:
        print("- **No GPU detected** (nvidia-smi not found, /dev/dri not present)")
    print()
